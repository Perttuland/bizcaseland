...
export function calculateEfficiencyGainsForMonth(businessData: BusinessData, monthIndex: number): number {
  ...
  for (const gain of efficiencyGains) {
    ...
    // CORRECTED LOGIC: Efficiency gains should represent the value generated by the improved process
    // Formula: Improved Value × Value per Unit × Implementation Factor
    // 
    // Example: 
    // - Improved process takes 8 hours/month
    // - Value per hour is 50 EUR/hour  
    // - Efficiency gain = 8 × 50 = 400 EUR/month (value of improved process)
    //
    // The cost savings (baseline - improved costs) are calculated separately in cost savings functions
    
    const monetaryValue = improvedValue * valuePerUnit * implementationFactor;
