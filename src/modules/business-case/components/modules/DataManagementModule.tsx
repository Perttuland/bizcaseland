...
import { safeJSONParse, validateBusinessData, exportBusinessCaseToPDF } from '@/core/services';
import { calculateBusinessMetrics, copyTextToClipboard } from '@/core/engine';
  ...
  const handlePDFExport = async () => {
    if (!businessData) {
      toast({
        ...
        description: "Please load business case data before exporting to PDF.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const calculations = calculateBusinessMetrics(businessData);
      await exportBusinessCaseToPDF(businessData, calculations);
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="copy-template" className="w-full">
        <TabsList className="grid w-full grid-cols-5">
          ...
          <TabsTrigger value="export-pdf" className="flex items-center gap-2 text-xs">
        ...
        {/* Export PDF Tab */}
        <TabsContent value="export-pdf" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Export as PDF Report</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {!businessData ? (
                <Alert>
                ...
                <>
                  <Alert>
                    <Info className="h-4 w-4" />
                    <AlertDescription>
                      Generate a professional PDF report with all your business case analysis.
                    </AlertDescription>
                  </Alert>
                  <Button 
